...
fun main() {
    ...
    val n = readLine()?.toIntOrNull()// Lê a entrada do usuário como uma String e converte para Inteiro ou retorna null se a conversão falhar

    // Verifica se o número inserido é válido (maior que zero)
    if (n != null && n > 0) {// Verifica se 'n' não é nulo e é maior que zero
        // Variável para armazenar o valor da soma
        var soma = 0.0

        // Calcula a soma conforme a fórmula fornecida e acumula na variável soma
        for (i in 1..n) {
            soma += i.toDouble() / (n - i + 1)// Calcula cada termo da série e adiciona à soma
        }
...
//Este código solicita ao usuário que insira um número inteiro positivo.
// Em seguida, verifica se o número é válido (ou seja, maior que zero).
// Se for válido, calcula a soma conforme a fórmula fornecida e imprime o resultado.
// Se o número não for válido, imprime uma mensagem de erro.